  ...
  withValidation(
    ...
    rateLimits.general(async (request: NextRequest) => {
      try {
        ...
        // Regular user registration - only allow customer role
        // Admin users should be created through the admin API
        const user = new User({
          email,
          password, // Password will be hashed by the User model pre-save hook
          name,
          role: 'customer', // Only allow customer role from registration
          emailVerified: true, // Set email as verified for easier testing
        });
